...
// NewCheckWithData returns a checkbox widget that is 2-way bound to the dataItem
func NewCheckWithData(data dataapi.DataItem, label string, callback func(checked bool)) *Check {

	// create the base widget
	w := NewCheck(label, callback)
	if bb, ok := data.(*dataapi.Bool); ok {
		w.SetChecked(bb.Value())
	}

	maskID := data.AddListener(func(b dataapi.DataItem) {
		if bb, ok := b.(*dataapi.Bool); ok {
			w.SetChecked(bb.Value())
			callback(bb.Value())
			return
		}
		// Bound to a number, so set the field based on 0 = false, else true
		if bb, ok := b.(*dataapi.Int); ok {
			value := bb.Value() > 0
			w.SetChecked(value)
			callback(value)
			return
